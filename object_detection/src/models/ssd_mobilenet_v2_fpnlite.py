...
def _bbox_predictor(fmap_channels, version, layer_in, n_anchors, n_offsets=4, kernel=(3, 3), l2_reg=0.0005, bn=False, dw=False):
    ...
    if version == 'v1':
        layer_in = _cascade4Conv(layer_in,fmap_channels,l2_reg,bn)
...
def _cls_predictor(fmap_channels, version, layer_in, n_anchors, n_classes, kernel=(3, 3), l2_reg=0.0005, bn=False, dw=False):
...
def _cascade4Conv(input,fmap_channels,l2_reg,bn):
...
def _cascade4DWConv(input,fmap_channels,l2_reg):
    output = _dw_conv(input, n_filters=fmap_channels, kernel=(3, 3), strides=(1, 1), depth_multiplier=1, padding='same', l2_reg=l2_reg, activation='relu6', bn=True, dilation_rate=(1, 1))
    for _ in range(3):
        output = _dw_conv(output, n_filters=fmap_channels, kernel=(3, 3), strides=(1, 1), depth_multiplier=1, padding='same', l2_reg=l2_reg, activation='relu6', bn=True, dilation_rate=(1, 1))
    return output

def _fmap_forward(fmap_channels, version, fmap, img_width, img_height, sizes, ratios, clip_boxes, normalize, n_classes, kernel=(3, 3), l2_reg=0.0005, bn=False, dw=False):
...
def _dw_conv(layer_in, n_filters, kernel=(3, 3), strides=(1, 1), depth_multiplier=1,padding='same', l2_reg=0.0005, activation='relu', bn=True, dilation_rate=(1, 1)):
    '''
...
def ssd_mobilenet_v2_fpnlite(
    ...
    if head_type == 'fpn':
        ...
        fmap_1 = _dw_conv(fmap_1, n_filters=fmap_channels, kernel=(3, 3), strides=(1, 1), depth_multiplier=1, padding='same', l2_reg=l2_reg, activation='relu6', bn=True, dilation_rate=(1, 1))
    ...
    if head_type == 'fpn':
        ...
        fmap_0 = _dw_conv(fmap_0, n_filters=fmap_channels, kernel=(3, 3), strides=(1, 1), depth_multiplier=1, padding='same', l2_reg=l2_reg, activation='relu6', bn=True, dilation_rate=(1, 1))
        fmap_3 = _dw_conv(x     , n_filters=fmap_channels, kernel=(3, 3), strides=(2, 2), depth_multiplier=1, padding='same', l2_reg=l2_reg, activation='relu6', bn=True, dilation_rate=(1, 1))
        fmap_4 = _dw_conv(fmap_3, n_filters=fmap_channels, kernel=(3, 3), strides=(2, 2), depth_multiplier=1, padding='same', l2_reg=l2_reg, activation='relu6', bn=True, dilation_rate=(1, 1))

        fmap_0_bis = _cascade4DWConv(fmap_0,fmap_channels,l2_reg)
        fmap_1_bis = _cascade4DWConv(fmap_1,fmap_channels,l2_reg)
        fmap_2_bis = _cascade4DWConv(x,     fmap_channels,l2_reg)
        fmap_3_bis = _cascade4DWConv(fmap_3,fmap_channels,l2_reg)
        fmap_4_bis = _cascade4DWConv(fmap_4,fmap_channels,l2_reg)
    ...
    for i in range(len(layers_list)):
        ...
        if head_type == 'fpn':
            anchors_i, cls_preds_i, bbox_preds_i = _fmap_forward(fmap_channels, version, layers_list[i], img_width, img_height, sizes[i], ratios[i],clip_boxes=clip_boxes, normalize=normalize, n_classes=n_classes,kernel=(3,3), l2_reg=l2_reg, bn=bn_pred, dw=False)
        elif head_type == 'fpn-lite':
            anchors_i, cls_preds_i, bbox_preds_i = _fmap_forward(fmap_channels, version, layers_list[i], img_width, img_height, sizes[i], ratios[i],clip_boxes=clip_boxes, normalize=normalize, n_classes=n_classes,kernel=(3,3), l2_reg=l2_reg, bn=bn_pred, dw=True)
