...
# - Validate from a local file:
#       - If path is from your local filesystem, it will upload it in Dev. Cloud and execute the command
print("Result from local file:", ai.validate(CliParameters(model=model_path)))
...
try:
    ...
    print("Result from cloud file:", ai.validate(CliParameters(model=model_name)))
