...
MDF_FilterConfigTypeDef Audio_MdfFilterConfig;
...
#endif
static int32_t Audio_DigMicRecBuff[DEFAULT_AUDIO_IN_BUFFER_SIZE] __NON_CACHEABLE;
/**
  * @}
  */
...
__weak HAL_StatusTypeDef MX_SAI1_ClockConfig(SAI_HandleTypeDef *hsai, uint32_t SampleRate)
{
  ...
  RCC_OscInitStruct.PLL2.PLLM = 8; /* 48/8 = 8MHz */
...
int32_t BSP_AUDIO_IN_Init_bak(uint32_t Instance, BSP_AUDIO_Init_t *AudioInit)
{
  int32_t ret = BSP_ERROR_NONE;
...
{
  GPIO_InitTypeDef  gpio_init_structure;
  static DMA_NodeTypeDef TxNode __NON_CACHEABLE;
  static DMA_NodeTypeDef RxNode __NON_CACHEABLE;
